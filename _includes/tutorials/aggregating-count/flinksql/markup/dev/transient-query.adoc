...
If you’re familiar with SQL, the text of the query itself is fairly self-explanatory. We are calculating the total number of records in each group, grouped by movie title. Note that `COUNT(total_ticket_value)` is still just counting the number of rows in the group, and is not doing any calculation based on ticket value itself. This query will keep running, continuing to return results until you hit CTRL-C.
