Now create a JDBC sink connector with the code below. Notice no transforms will be necessary to the timestamp column because the column is already a supported `TIMESTAMP` type.
