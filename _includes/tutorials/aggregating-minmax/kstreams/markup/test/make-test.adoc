...
This test file uses the `https://kafka.apache.org/{{ site.ak_javadoc_version }}/javadoc/org/apache/kafka/streams/TestInputTopic.html[TestInputTopic]` and `https://kafka.apache.org/{{ site.ak_javadoc_version }}/javadoc/org/apache/kafka/streams/TestOutputTopic.html[TestOutputTopic]` testing features introduced in Kafka 2.4.   These features simplify testing including the ability to test inputs and outputs 'record-by-record', for example:
+++++
