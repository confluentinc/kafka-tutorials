...
public class FlinkSqlAggregatingMinMaxTest extends AbstractFlinkKafkaTest {
  ...
  public void simpleSelect() throws Exception {
    ...
    tEnv.executeSql(getResourceFileContents("create-movie-sales.sql.template",
        Optional.of(kafkaPort), Optional.of(schemaRegistryPort))).await();
    tEnv.executeSql(getResourceFileContents("populate-movie-sales.sql"));
    tEnv.executeSql(getResourceFileContents("create-movie-sales-by-year.sql.template",
        Optional.of(kafkaPort), Optional.of(schemaRegistryPort))).await();
    ...
    tEnv.executeSql(getResourceFileContents("populate-movie-sales-by-year.sql"));

    // execute query on result table that should have movie sales aggregated by release year
    TableResult tableResult = tEnv.executeSql(getResourceFileContents("query-movie-sales-by-year.sql"));
