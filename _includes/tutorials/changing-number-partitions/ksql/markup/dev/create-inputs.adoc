First, you'll need to create a ksqlDB stream for the original topic. The statement below specifies the message value serialization format is `JSON` but in your environment, `VALUE_FORMAT` should be set to match the https://docs.ksqldb.io/en/latest/developer-guide/serialization/#serialization-formats[serialization format] of your original topic.
