Run the Kafka producer performance test again, but this time use optimized configuration values.
This test will send the same number of records of the same size as the previous performance test.
...
While this test has demonstrated a starting point for improving the producer throughput, you should do further testing in your environment of these configuration parameters, and test it with your specific Kafka producer, not just using `kafka-producer-perf-test`.
